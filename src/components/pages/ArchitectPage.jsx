            ...
            <div className="p-4">
              ...
              {isLoadingClients ? (
                ...
                <div className="space-y-2 max-h-[400px] overflow-y-auto">
                  {(() => {
                    // Group clients by teamId; clients with null teamId bucket into "Personal".
                    const groups = new Map();
                    for (const client of commandCenterClients) {
                      const key = client.teamId || '__personal__';
                      const label = client.teamId ? (client.teamName || 'Team') : 'Personal';
                      if (!groups.has(key)) groups.set(key, { label, clients: [] });
                      groups.get(key).clients.push(client);
                    }
                    // Stable order: real teams first (alphabetized), Personal last.
                    const entries = [...groups.entries()].sort((a, b) => {
                      if (a[0] === '__personal__') return 1;
                      if (b[0] === '__personal__') return -1;
                      return a[1].label.localeCompare(b[1].label);
                    });
                    return entries.map(([key, group]) => (
                      <div key={key}>
                        <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider px-1 pt-1 pb-1">
                          {group.label}
                        </div>
                        {group.clients.map((client) => (
                          ...
                          >
                            ...
                            {client.ownerAdvisorName && client.ownerAdvisorId !== commandCenterAdvisorId && (
                              <div className="text-xs text-slate-400 mt-0.5">Advisor: {client.ownerAdvisorName}</div>
                            )}
                          </button>
                        ))}
                      </div>
              ...
              <button
                onClick={async () => {
                  ...
                  const clientTeam = selectedCommandCenterClient.teamId
                    ...
                    : null;
                  const ownerAdvisorId = selectedCommandCenterClient.ownerAdvisorId || null;
                  const result = await onSaveToCommandCenter(selectedCommandCenterClient.id, ownerAdvisorId, clientTeam);
                  if (result.success) {
