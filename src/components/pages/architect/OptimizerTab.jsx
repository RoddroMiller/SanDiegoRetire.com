...
export const OptimizerTab = ({ optimizerData, inputs, basePlan, monteCarloData, projectionData, optimizerRebalanceFreq, onSetOptimizerRebalanceFreq, clientInfo, assumptions, vaEnabled, vaInputs, vaOptimizerData }) => {
  ...
  const optimizerLegacyYear = new Date().getFullYear() + Math.max(0, optimizerFinalAge - (clientInfo?.currentAge || clientInfo?.retirementAge || 65));
  const optimizerLegacyLabel = `${optimizerLegacyYear}`;
    ...
    return (
      ...
      >
        ...
        <div className="mb-4 p-3 bg-slate-100 rounded-lg">
          <p className="text-xs text-slate-500">Projected Legacy Balance {optimizerLegacyLabel}</p>
